...
const Round2 = () => {
    ...
    useEffect(() => {
        ...
        const storedSubmitedQuestions = JSON.parse(localStorage.getItem("submitedQuestions")) || [];
    ...
    useEffect(() => {
        ...
        localStorage.setItem("submitedQuestions", JSON.stringify(submitedQuestions));
    ...
    const handleSubmit = () => {
        if (submitedQuestions.includes(questionIndex)) {
            alert("You have already submitted your answer for this question. You cannot modify it.");
            return; // Prevent submitting again
        }

        // Show a confirmation message before submitting
        const confirmSubmit = window.confirm("Once you submit your answer, you cannot modify it. Are you sure you want to submit?");
        if (confirmSubmit) {
    ...
    return (
        <>
            <div className="flex justify-evenly items-center h-screen">
                {/* question container */}
                <div className="bg-gray-300 rounded-2xl p-5 h-[80vh] w-[40vw] overflow-y-scroll">
                    {/* top section */}
                    <div className="flex items-center justify-evenly" style={{ marginTop: "10px" }}>
                        <img src={itUtsavLogo} alt="" className="h-[5rem]" />
    ...
    style={{
        ...
        paddingTop: "1px",
        paddingBottom: "1px",
        backgroundColor: submitedQuestions.includes(0) ? '#28a745' : (questionIndex === 0 ? '#ffeb3b' : '') // Green if submitted, yellow if selected
    }}
    onClick={() => {
        setQuestionIndex(0);
        setCode("");
...
<div
    ...
    style={{
        ...
        paddingTop: "1px",
        paddingBottom: "1px",
        backgroundColor: submitedQuestions.includes(1) ? '#28a745' : (questionIndex === 1 ? '#ffeb3b' : '') // Green if submitted, yellow if selected
    }}
    onClick={() => {
        setQuestionIndex(1);
        setCode("");
...
<div
    ...
    style={{
        ...
        paddingTop: "1px",
        paddingBottom: "1px",
        backgroundColor: submitedQuestions.includes(2) ? '#28a745' : (questionIndex === 2 ? '#ffeb3b' : '') // Green if submitted, yellow if selected
    }}
    onClick={() => {
        setQuestionIndex(2);
        setCode("");
...
>
    Question 3
                    ...
                    {/* question */}
                    <div className="text-black text-justify " style={{ padding: "2rem", marginTop: "1rem" }}>
                ...
                <div className="bg-gray-300 rounded-2xl p-5 h-[80vh] w-[40vw] ml-3 flex items-center justify-center">
                    <div className="h-[75vh] w-[37vw] bg-gray-500 rounded-2xl">
                        {/* top bar */}
                        ...
                        <div className="flex items-center justify-center">
                            <CodeEditor value={code} setValue={setCode} selectedLanguage={currentLanguage} />
                        </div>
                    </div>
                </div>
            </div>
