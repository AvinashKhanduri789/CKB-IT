...
const Round2 = () => {
  ...
  const handleSubmit = () => {
    ...
    if (confirmSubmit) {
      ...
      submitAnswer(teamName, code, currentLanguage,questionIndex+1,questions[questionIndex]._id, submissionTime)
        .then((response) => {
  ...
  return (
        ...
        <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl p-4 md:p-5 w-full md:w-[40%] flex flex-col shadow-lg border border-gray-600">
          ...
          <div className="flex justify-between items-center rounded-lg bg-gray-700 p-1 mb-4 md:mb-5 border border-gray-500">
            {[0, 1, 2].map((index) => (
              <div
                key={index}
                className={`font-bold text-xs md:text-sm rounded-lg px-2 md:px-3 py-1 cursor-pointer transition-all duration-300 flex-1 text-center mx-1 ${submitedQuestions.includes(index)
          ...
          <div className="bg-gray-700 rounded-lg p-4 flex-1 overflow-y-auto border border-gray-600">
            <div className="text-white">
              ...
              {questions[questionIndex]?.testCases?.filter(tc => tc.isPublic).length > 0 && (
                <div className="mt-6 space-y-4">
                  <h3 className="font-semibold text-blue-300 text-lg">Examples:</h3>

                  {questions[questionIndex].testCases
              ...
              {/* Show difficulty and score */}
              {questions[questionIndex] && (
                <div className="mt-6 flex flex-wrap gap-4">
                  <div className="flex items-center gap-2">
                    <span className="text-gray-400 text-sm">Difficulty:</span>
                    <span className={`px-2 py-1 rounded text-xs font-medium ${questions[questionIndex].difficulty === 'easy' ? 'bg-green-900/50 text-green-300' :
                      questions[questionIndex].difficulty === 'medium' ? 'bg-yellow-900/50 text-yellow-300' :
                        'bg-red-900/50 text-red-300'
                      }`}>
                      {questions[questionIndex].difficulty.charAt(0).toUpperCase() + questions[questionIndex].difficulty.slice(1)}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-2 py-1 rounded text-xs font-medium bg-blue-900/50 text-blue-300">
                      {questions[questionIndex].maxScore} points
                    </span>
                  </div>
                </div>
              )}
    ...
    ${isSubmitting ? 'opacity-75 cursor-not-allowed' : ''}`}
                  ...
                  {isSubmitting ? (
                    ...
                    <>
                      <svg
                        ...
                        className="h-4 w-4 md:h-5 md:w-5 mr-2 group-hover:animate-bounce"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
            ...
            <div className="mt-3 flex justify-center">
              <div className={`px-3 py-1 rounded-md text-xs font-semibold flex items-center ${submitedQuestions.includes(questionIndex)
